...
- `samples/` contains 62 single-note sustained "ah" vocal WAV files (male C2–C5, female C3–C5) with `tone_map.json` metadata. Generator lives in `crates/sample-gen/`. All files are synthetically generated (formant synthesis), original to the project.
...
- **CLI mode** (`crates/ferrotone-cli/`, zero Tauri dep) exists for batch WAV processing and timed mic capture. Any non-GUI feature (DSP pipeline changes, new detectors, gate modifications, music utilities) must be tested via `cargo run --bin ferrotone-cli -- -f samples/<note>.wav` before being declared done. This ensures the core crate works correctly without the Tauri/GUI layer.
